...
def check_rh_budget(input_filename, tmp_path, stella_version, tolerance,
    ...
    (time, E_RH, dE_RH_dt, P_RH, P_nonlinear, P_collisional, P_drift, P_drift_tr,
     P_drift_pa, P_induction) = get_rh_budget(
        local_netcdf_file, time_min, time_max, kx_max)

    if channel == 'nonlinear':
        measured, expected, what = (dE_RH_dt - P_collisional - P_drift - P_induction,
                                    P_nonlinear, 'nonlinear channel')
    else:
...
#-------------------------------------------------------------------------------
def check_rh_omega_budget(input_filename, tmp_path, stella_version, tolerance,
    ...
    time, E, dE_dt, P, P_nl, P_coll, P_drift = get_rh_omega_budget(
        local_netcdf_file, time_min, time_max, kx_max)
...
def test_whether_umom_budget_closes_for_linear_collisional_zonal_flow(tmp_path, stella_version):
    ...
    '''
    check_rh_omega_budget('rh_linear_collisional.in', tmp_path, stella_version,
                         tolerance=0.05)
    return
