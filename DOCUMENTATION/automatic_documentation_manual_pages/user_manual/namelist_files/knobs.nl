...
`enforce_periodic_fields`  | boolean | `false` | Make the fields returned by the field solve periodic for the periodic (zonal) modes, by overwriting their value at \\( z = +\pi \\) with the (phase-shifted) value at \\( z = -\pi \\), as is already done for the fields obtained from the response matrix. The distribution function of a zonal mode is identified between the two ends of the flux tube, but the field solve is local in \\( z \\); on a flux tube that does not close on itself (different \\( k_\perp^2 \\) and \\( B \\) at the two ends) the fields at the two ends then differ, which puts a spurious parallel electric field in the last cell of the time-centred streaming source and drives a numerical instability of the zonal mode with kinetic electrons. No effect on a flux tube that closes on itself.
